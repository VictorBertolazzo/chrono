...
#ifndef WL_SIMPLEMAPPOWERTRAIN_H
#define WL_SIMPLEMAPPOWERTRAIN_H
...
/// Custom powertrain model for a WL vehicle.
/// Note that this does not use one of the Chrono::vehicle templates, but rather inherits directly
/// from ChPowertrain.
class WL_SimpleMapPowertrain : public chrono::vehicle::ChPowertrain {
  public:
    WL_SimpleMapPowertrain();
    ...
    /// This simplified model does not have a transmission box.
    virtual void SetDriveMode(chrono::vehicle::ChPowertrain::DriveMode mmode) override;
    ...
    virtual void Initialize(std::shared_ptr<chrono::ChBody> chassis,     ///< [in] chassis o the associated vehicle
                            std::shared_ptr<chrono::ChShaft> driveshaft  ///< [in] shaft connection to the vehicle driveline
                            ) override;
  ...
  private:
    ...
    chrono::ChCubicSpline m_zeroThrottleMap;
    chrono::ChCubicSpline m_fullThrottleMap;
};
