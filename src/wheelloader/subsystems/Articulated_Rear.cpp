...
const ChVector<> Articulated_Rear::m_chassisCOM(-0.5, 0, 2.0);          // COM location
const ChVector<> Articulated_Rear::m_chassisInertia(800, 800, 900);  // chassis inertia (roll,pitch,yaw)
...
void Articulated_Rear::Initialize() {
	...
	m_joint->Set_eng_mode(ChLinkEngine::ENG_MODE_ROTATION);//ENG_MODE_TORQUE
...
void Articulated_Rear::Synchronize(double time, double steering, double braking, const TireForces& tire_forces) {
    ...
    // Apply steering
// STEERING
	double max_angle = 40 * CH_C_DEG_TO_RAD;
